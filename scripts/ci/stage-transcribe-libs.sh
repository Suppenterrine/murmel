...
#
# On Linux/Windows, Murmel builds transcribe-cpp in its `dynamic-backends`
...
# them. This script copies them, as a co-located set, next to where the `murmel`
# binary will find them at runtime:
#   - the loader resolves `libtranscribe` via the $ORIGIN-relative rpath baked
#     into `murmel` (see src-tauri/build.rs);

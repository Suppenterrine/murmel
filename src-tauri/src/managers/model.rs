...
/// Where a model comes from and how Murmel obtains it — the routing discriminant
...
/// The base code Murmel matches a language *intent* on: a tag's primary subtag,
...
/// Bridges hf-hub's async download progress to Murmel's `model-download-progress`
...
impl ModelManager {
    ...
    /// models dropped in or downloaded outside Murmel show up without a restart.
    ...
    fn discover_custom_transcribe_models(
        ...
        for entry in fs::read_dir(models_dir)? {
            ...
            // Murmel-downloaded one. Legacy `.bin` files have no GGUF header, so
    ...
    /// shared Hugging Face cache, so models downloaded by Murmel (or any other

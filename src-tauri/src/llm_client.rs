...
fn build_headers(provider: &PostProcessProvider, api_key: &str) -> Result<HeaderMap, String> {
    ...
    headers.insert(
        REFERER,
        HeaderValue::from_static("https://github.com/Suppenterrine/murmel"),
    );
    headers.insert(
        USER_AGENT,
        HeaderValue::from_static("Murmel/1.0 (+https://github.com/Suppenterrine/murmel)"),
    );
    headers.insert("X-Title", HeaderValue::from_static("Murmel"));
